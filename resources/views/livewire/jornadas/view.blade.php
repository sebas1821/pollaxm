		...
		<div class="col-md-12">
			<div class="card shadow p-3 mb-5 bg-body rounded">
				<div class="">
					<div style="display: flex; justify-content: space-between; align-items: center;">
						<div class="float-left">
							...
							Lista De Jornadas </h4>
						</div>
						@if (session()->has('message'))
						<div wire:poll.4s class="btn btn-sm btn-success" style="margin-top:0px; margin-bottom:0px;"> {{ session('message') }} </div>
						@endif
						<div>
							<input wire:model='keyWord' type="text" class="form-control" name="search" id="search" placeholder="Buscar Jornadas">
						</div>
						<div class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#createDataModal">
						<i class="fa fa-plus"></i>  Agregar Jornadas
						</div>
					</div>
				</div>
				<br>
				...
				<div class="table-responsive">
					<table class="table table-bordered table-sm">
						<thead class="thead">
							<tr> 
								<td>#</td> 
								<th>Descripcion</th>
								<th>Puntaje Marcador Exacto</th>
								<th>Puntaje Ganador </th>
								<td>Acciones</td>
							</tr>
						</thead>
						<tbody>
							...
							<tr>
								...
								<td>{{ $row->valor_puntaje_me }}</td>
								<td>{{ $row->valor_puntaje_g }}</td>
								<td width="90">
									<div class="dropdown">
					...
					<div class="float-end">{{ $jornadas->links() }}</div>
					</div>
				</div>
			</div>
		</div>
...
<script>
    ...
    Livewire.on('eliminar', rId => {
            ...
            Swal.fire({
                title: '',
                text: "Seguro que desea eliminar la jornada?",
                icon: 'warning',
